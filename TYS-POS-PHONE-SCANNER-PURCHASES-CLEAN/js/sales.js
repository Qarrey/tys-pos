...
let lastCameraBarcodeTime = 0;

function setCameraScannerStatus(message) {
    const status = document.getElementById("camera-scanner-status");
    if (status) status.textContent = message;
}

function stopCameraBarcodeScanner() {
    cameraBarcodeScanning = false;

    if (cameraBarcodeFrame) {
        cancelAnimationFrame(cameraBarcodeFrame);
        cameraBarcodeFrame = null;
    }

    if (cameraBarcodeStream) {
        cameraBarcodeStream.getTracks().forEach(track => track.stop());
        cameraBarcodeStream = null;
    }
    ...
    const button = document.getElementById("camera-scan-btn");
    if (button) {
...
function useCameraBarcode(barcode) {
    const cleanBarcode = String(barcode || "").trim();
    if (!cleanBarcode) return;

    const now = Date.now();
    if (cleanBarcode === lastCameraBarcode && now - lastCameraBarcodeTime < 1500) {
        return;
    }

    lastCameraBarcode = cleanBarcode;
    lastCameraBarcodeTime = now;

    const input = document.getElementById("barcode-input");
    if (!input) {
        stopCameraBarcodeScanner();
        alert("The barcode input was not found.");
        return;
    }
...
async function detectCameraBarcode() {
    if (!cameraBarcodeScanning || !cameraBarcodeDetector) return;

    const video = document.getElementById("camera-scanner-video");
    if (!video || video.readyState < 2) {
        cameraBarcodeFrame = requestAnimationFrame(detectCameraBarcode);
        return;
    }

    try {
        const barcodes = await cameraBarcodeDetector.detect(video);
        if (barcodes && barcodes.length) {
            const barcode = barcodes[0].rawValue;
            if (barcode) {
                setCameraScannerStatus(`Barcode detected: ${barcode}`);
                useCameraBarcode(barcode);
                return;
            }
        }
    } catch (error) {
...
async function startCameraBarcodeScanner() {
    if (!window.isSecureContext) {
        alert("Camera scanning requires the secure HTTPS version of the POS.");
        return;
    }
    ...
    if (!("BarcodeDetector" in window)) {
        alert("This browser does not support camera barcode detection. Use Chrome on Android, or use the USB/manual scanner.");
        return;
    }
    ...
    if (!panel || !video || !button) {
        alert("The camera scanner controls are missing from index.html.");
        return;
    }
    ...
    try {
        ...
        panel.hidden = false;
        setCameraScannerStatus("Allow camera access, then point the rear camera at the barcode.");

        cameraBarcodeStream = await navigator.mediaDevices.getUserMedia({
            audio: false,
            video: {
        ...
        video.srcObject = cameraBarcodeStream;
        await video.play();
        ...
        cameraBarcodeDetector = formats.length
            ? new BarcodeDetector({ formats })
            : new BarcodeDetector();

        cameraBarcodeScanning = true;
        button.textContent = "Camera Open";
        setCameraScannerStatus("Keep the barcode clear, level and steady inside the camera view.");
        detectCameraBarcode();
    } catch (error) {
        ...
        if (error.name === "NotAllowedError") {
            alert("Camera permission was denied. Allow camera access in your browser settings and try again.");
        } else if (error.name === "NotFoundError") {
            alert("No camera was found on this device.");
        } else {
            alert(`Could not open the camera: ${error.message || "Unknown camera error"}`);
        }
    }
}

function initializeCameraBarcodeScanner() {
    const openButton = document.getElementById("camera-scan-btn");
    const closeButton = document.getElementById("close-camera-scanner-btn");

    if (openButton && !openButton.dataset.ready) {
        openButton.dataset.ready = "true";
        openButton.addEventListener("click", startCameraBarcodeScanner);
    }

    if (closeButton && !closeButton.dataset.ready) {
        closeButton.dataset.ready = "true";
        closeButton.addEventListener("click", stopCameraBarcodeScanner);
    }

    if (!document.body.dataset.cameraVisibilityReady) {
        document.body.dataset.cameraVisibilityReady = "true";
        document.addEventListener("visibilitychange", () => {
            if (document.hidden && cameraBarcodeScanning) {
                stopCameraBarcodeScanner();
            }
        });
    }
