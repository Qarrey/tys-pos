...
async function adjustStock(
    ...
    if (typeof updateProductStockInSupabase !== "function") {
        alert("Stock synchronization module is unavailable.");
        return;
    }

    const result = await updateProductStockInSupabase(
        product.id,
        newStock,
        {
    ...
    if (result.queued) {
        showFeedback(
            "Stock saved on this device. It will sync when internet returns."
        );
        alert(
            "Stock updated offline and added to the synchronization queue."
        );
    } else {
        showFeedback("Stock updated locally and online.");
    }
}
