...
function addToCart(productId, variantId = "") {
    ...
    if (variantId && (!variant || String(variant.productId) !== String(product.id) || variant.active === false)) {
        alert("That product variation is no longer available.");
        return;
    }
    ...
    if (existing) {
        ...
        cart.push({
            ...
            variationName: variant ? variant.name : "",
...
function addProductByBarcode() {
    ...
    const variation = typeof getVariationByBarcode === "function"
        ? getVariationByBarcode(barcode)
        : null;

    if (variation) {
        addToCart(variation.productId, variation.id);
        input.value = "";
        input.focus();
        return;
    }
    ...
    if (!product) {
        alert("Product or variation not found.");
        input.select();
        return;
    }
