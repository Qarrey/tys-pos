...
async function loadProductVariations() {
    ...
    window.dispatchEvent(new CustomEvent("product-variations-updated", {
        detail: { variations: window.productVariations }
    }));
...
async function upsertProductVariation(variation) {
    ...
    const payload = {
        ...
        barcode: String(variation.barcode || "").trim() || null,
    ...
    if (variationId) {
        ...
        const updated = getVariationById(variationId);

        if (!updated) {
            throw new Error("Variation was not found after updating. Refresh and try again.");
        }
    ...
    if (saved) return normalizeVariation(saved);

    const created = (window.productVariations || []).find(item =>
        String(item.productId) === String(variation.productId) &&
        item.name.toLowerCase() === payload.name.toLowerCase()
    );

    if (!created) throw new Error("Variation could not be created.");
    return created;
}
...
window.addEventListener("product-variations-updated", () => {
    if (typeof renderProducts === "function") {
        renderProducts(document.getElementById("search-input")?.value || "");
    }
});
